...
# basic map plot
plot_base <- ggplot() + theme_bw(base_size = 7) +
  theme(panel.background = element_rect(fill = col_sea),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        strip.background = element_blank(),
        strip.text = element_text(angle = 0, hjust = 0, size = 8))
...
# save raw plot grob
saveRDS(plot_combined, "figure3_PCA_maps/figure3_part1.rds")

# save to file
file_ext <- c("eps", "pdf", "png")
for (i in seq_along(file_ext)) {
  ggsave(sprintf("figure3_PCA_maps/figure3_part1.%s", file_ext[i]),
         plot = plot_combined, device = file_ext[i], width = 114, height = 100, units = "mm")
}
